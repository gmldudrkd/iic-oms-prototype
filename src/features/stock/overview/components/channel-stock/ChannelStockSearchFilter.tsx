...
import {
  ...
  SHOW_ONLY_PRE_ORDER_STOCK_LIST,
...
interface ChannelStockSearchFilterProps {
...
export default function ChannelStockSearchFilter({
  control,
  channelList,
  onSearch,
  onReset,
}: ChannelStockSearchFilterProps) {
  return (
    <Box
      ...
      >
        <Box display="flex" gap="10px" flexWrap="wrap">
          ...
          <FormControl sx={{ width: "150px" }}>
            <CustomSelectCheckbox
              control={control}
              selectList={CHANNEL_SEND_STATUS_LIST}
              placeholder="Select"
              labelName="Channel Send Status"
              name="channelSendStatus"
              enableAllOption={true}
              selectProps={{
                multiple: true,
              }}
          ...
          <FormControl sx={{ width: "200px" }}>
            <Controller
              control={control}
              name="hasPreorderQuantity"
              render={({ field }) => (
                <>
                  <InputLabel shrink={true}>Pre-order Stock Filter</InputLabel>
                  <Select {...field} label="Pre-order Stock Filter">
                    {SHOW_ONLY_PRE_ORDER_STOCK_LIST.map((item) => (
                      <MenuItem
