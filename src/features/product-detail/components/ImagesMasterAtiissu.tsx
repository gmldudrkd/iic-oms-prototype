...
import { validateImageFilesAtiissu } from "@/features/product-detail/modules/utils";
...
export default function ImagesMasterAtiissu({
  modelCode,
  sapCode,
  uploadedImages,
  setUploadedImages,
  ...
  const handleFileChange = useCallback(
      ...
      if (files && files.length > 0) {
        ...
        const loadImage = (file: File) =>
          new Promise<UploadedImage>((resolve, reject) => {
            ...
            img.onload = () => {
              if (img.width !== 3600 && img.height !== 3600) {
                setOpenErrorDialog(
                  "Invalid image size. Only images with 3600x3600 resolution are allowed.",
                );
                reject(`${file.name}의 크기가 3600x3600이 아닙니다.`);
        ...
        try {
          ...
          const result = validateImageFilesAtiissu(
            newImages,
            uploadedImages,
            modelCode,
            sapCode,
    ...
    [uploadedImages, modelCode, sapCode, setUploadedImages],
  );
